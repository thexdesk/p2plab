...
func (p *Peer) GraphSync(ctx context.Context, c cid.Cid, targetPeer peer.ID) error {
	// selector := p.ssb.ExploreRecursive(100,
	// 	p.ssb.ExploreFields(func(efsb selectorbuilder.ExploreFieldsSpecBuilder) {
	// 		efsb.Insert("Parents", p.ssb.ExploreAll(
	// 			p.ssb.ExploreRecursiveEdge()))
	// 	}),
	// ).Node()

	selector := p.ssb.ExploreRecursive(10,
		p.ssb.ExploreFields(func(efsb selectorbuilder.ExploreFieldsSpecBuilder) {
			efsb.Insert("links", p.ssb.ExploreAll(
				p.ssb.ExploreFields(func(efsb selectorbuilder.ExploreFieldsSpecBuilder) {
					efsb.Insert("Cid", p.ssb.ExploreRecursiveEdge())
				}),
			))
		}),
	).Node()

	progressChan, errChan := p.gsync.Request(ctx, targetPeer, cidlink.Link{Cid: c}, selector)
	for err := range errChan {
		return err
	}

	fmt.Println("Loading progress chan")
	for progress := range progressChan {
		link := progress.Node.Length()
		fmt.Printf("Received ipld node over graphsync: %d\n", link)
	}
	fmt.Println("Finished progress chan")
